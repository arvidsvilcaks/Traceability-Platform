<x-app-layout>
  ...
  <div class="overflow-x-auto shadow-md sm:rounded-lg mt-6">
            ...
            <tbody>
                @foreach ($traceabilityWholesaler as $traceabilityWholesaler)
                    <tr>
                        ...
                        <td class="px-6 py-4 border">{{ $traceabilityWholesaler->addressWholesaler}}</td>
                        <td class="px-6 py-4 border">
                        <button onclick="editModalTrace({{ json_encode($traceabilityWholesaler) }})" class="bg-gray-500 text-white px-2 py-1 mb-4 rounded-full">Edit</button>
    ...
    <div id="addModalTrace" class="fixed inset-0 flex items-center justify-center hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form action="{{ route('traceabilityWholesaler.storeWholesalerTrace', $honeyInfo->id) }}" method="POST">
                @csrf
                <input type="hidden" name="product_id" value="{{ $honeyInfo->id }}">
                <div class="flex flex-col space-y-2">
                    <input type="date" name="dateWholesaler" id="add_dateWholesaler" placeholder="Date" required>
                    <input type="text" name="addressWholesaler" id="add_addressWholesaler" placeholder="Address" required>
    ...
    <div id="editModalTrace" class="fixed inset-0 flex items-center justify-center hidden">
        ...
        <h2 class="text-lg font-semibold mb-4">Edit Trace</h2>
            <form id="editTraceForm" action="{{ route('traceabilityWholesaler.updateWholesalerTrace', ':id') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
  ...
  <div class="overflow-x-auto shadow-md sm:rounded-lg mt-6">
    <table class="w-full text-sm text-center text-gray-500 border-separate border border-gray-200">
      ...
      @foreach($qualityWholesaler as $qualityWholesaler)
        <tr>
          <td class="px-6 py-4 border">{{ $qualityWholesaler->quality_standard }}</td>
  ...
  <div class="overflow-x-auto shadow-md sm:rounded-lg mt-6 mb-6 bg-gray-100">
    <table class="w-full text-sm text-center text-gray-500 border-separate border border-gray-200">
      ...
      @foreach($market as $market)
        <tr>
          <td class="px-6 py-4 border">{{ $market->market_name }}</td>
          <td class="px-6 py-4 border">{{ $market->market_location }}</td>
          <td class="px-6 py-4 border">
  ...
  <script>
    ...
    function editModalTrace($traceabilityWholesaler) {
        document.getElementById('edit_TraceId').value = $traceabilityWholesaler.id;
        document.getElementById('add_dateWholesaler').value = $traceabilityWholesaler.dateWholesaler;
        document.getElementById('add_addressWholesaler').value = $traceabilityWholesaler.addressWholesaler;

        let form = document.getElementById('editTraceForm');
        form.action = form.action.replace(':id', $traceabilityWholesaler.id);
