        ...
        <div class="overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <tbody>
                ...
                <td class="px-6 py-4 border">
                    ...
                    @endif
                    <div class="mt-4">
                        <form action="{{ route('processesPackaging.destroyVisualMaterial', $processesPackaging->id) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <div id="editModalPackages" class="fixed inset-0 flex items-center justify-center hidden">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form id="editPackagesForm" action="{{ route('packages.updatePackage', ':id') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="flex flex-col space-y-2">
                    <input type="number" name="quantity" id="edit_quantity" placeholder="Quantity" class="border p-2 rounded" required>
                    <input type="text" name="type" id="edit_type" placeholder="Type" class="border p-2 rounded" required>
                    <input type="number" name="package_weight" id="edit_package_weight" placeholder="Package Weight (kg)" class="border p-2 rounded" required>
                    ...
                    <select name="market_id" id="edit_market_id" class="border p-2 rounded" required>
                        <option value="">Select Market</option>
                        @foreach($markets as $market)
                            <option value="{{ $market->id }}">{{ $market->name }}</option>
                        @endforeach
